...
size_t load(struct HashMap *m, size_t nw)
{
    ...
    size_t nwords, msize, slen, cnt, retval;
    ...
    while ((nwords < nw) && (nwords < msize))
    {
        ...
        if (fgets(val, MAX_WORD_SIZE + 1, dict))
        {
            ...
            // Only add words greater than the minimum.
            if (slen > MIN_WORD_SIZE)
            {
                lstr(val);
                strcpy(key, val);

                qsort(key, slen, sizeof(char), cmpfnc);
                retval = addpair(m, key, val);

                if (!retval || retval >= H_COL) {cnt++;}
            }
