...
size_t subword(struct HashMap *m, char *s)
{
    ...
    midpt = slen = s ? strlen(s) : 0;

    if (slen > 1)
    {
        // As quicksort sorts in-place, we need to store
        // the sorted and unsorted string for each.
        w1[0] = malloc(sizeof(char) * slen);
        w1[1] = malloc(sizeof(char) * slen);
        w2[0] = malloc(sizeof(char) * slen);
        w2[1] = malloc(sizeof(char) * slen);
