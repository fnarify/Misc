...
int addpair(struct HashMap *m, char *key, char *val)
{
    ...
    else if (samelength(klen, m->keys[hashval]))
    {
        ...
        if (!strcmp(key, m->keys[hashval])) {return addvalue(m, hashval, val);}
    }
    else
    {
    ...
    // Take a hash and check, otherwise take the next hash.    
    i = 0;
    while (i < NUM_HASHES)
    {
        ...
        // Point function pointer to new hash.
        hash = &hash2;
        i++;
    }

    // Collision, so iterate through the hashmap.
    i = 0;
    do
    {
        if (!m->keys[i])
        ...
        else if (samelength(klen, m->keys[i]))
        {
            if (!strcmp(key, m->keys[i])) 
            {
                return addvalue(m, i, val);
...
struct LinkedWords *find(struct HashMap *m, char *k)
{
    ...
    while (i < NUM_HASHES)
    {
        ...
        if (m->keys[hashval])
        {
            if (klen == strlen(m->keys[hashval]))
            {
                if (!strcmp(k, m->keys[hashval])) {return m->values[hashval].words;}
            }
        }
    ...
    // Collision, so now we have to iterate through the hashmap.
    // Extremely slow for large amount of collisions.
    i = 0;
    do
    {
        if (m->keys[i])
        {
            if (klen == strlen(m->keys[i]))
            {
                if (!strcmp(k, m->keys[i])) {return m->values[i].words;}
            }
        }
